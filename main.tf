...
resource "aws_instance" "swarm_master" {
  ...
  provisioner "file" {
    source      = var.key_path
    destination = "/home/ubuntu/devops.pem"
...
resource "null_resource" "swarm_join_worker" {
  ...
  provisioner "remote-exec" {
    inline = [
      "docker swarm join --token $(ssh -o StrictHostKeyChecking=no -i /home/ubuntu/devops.pem ubuntu@${aws_instance.swarm_master[0].public_ip} 'docker swarm join-token worker -q') ${aws_instance.swarm_master[0].public_ip}:2377"
    ]
